...
class FeatureController extends Controller
{
    ...
    public function updateSeekerProfile(Request $request)
    {
        $validated = $request->validate([
            ...
            'bday_month' => ['nullable', 'string', 'in:Jan,Feb,Mar,Apr,May,Jun,Jul,Aug,Sep,Oct,Nov,Dec'],
            'bday_day' => ['nullable', 'integer', 'between:1,31'],
            'bday_year' => ['nullable', 'integer', 'min:1900', 'max:2100'],
            'gender' => ['nullable', 'string', 'in:Male,Female,Binary,Other'],
        ]);
        ...
        $month = (string) ($validated['bday_month'] ?? $seeker->bday_month ?? '');
        $day = (int) ($validated['bday_day'] ?? $seeker->bday_day ?? 0);
        $year = (int) ($validated['bday_year'] ?? $seeker->bday_year ?? 0);
        $gender = trim((string) ($validated['gender'] ?? $seeker->gender ?? ''));

        if ($month === '' || $day < 1 || $year < 1900 || $gender === '') {
            return response()->json([
                'status' => 'error',
                'message' => 'Birthday and gender are required.',
            ], 422);
        }
        ...
        if (!isset($monthMap[$month]) || !checkdate($monthMap[$month], $day, $year)) {
            return response()->json([
                'status' => 'error',
        ...
        $birthDate = Carbon::createFromDate($year, $monthMap[$month], $day)->startOfDay();
    ...
    public function getSavedJobs(Request $request)
    {
        ...
        if (($seeker->role ?? '') !== 'Seeker') {
            return response()->json(['status' => 'error', 'message' => 'Only seekers can access saved jobs.'], 422);
        }
    ...
    public function toggleSaveJob(Request $request)
    {
        ...
        if (($seeker->role ?? '') !== 'Seeker') {
            return response()->json(['status' => 'error', 'message' => 'Only seekers can save jobs.'], 422);
        }
