...
const Login = ({ onLogin }) => {
  ...
  const handleQcIdChange = (e) => {
    ...
    const handleQcIdInputChange = (e) => {
        ...
        const nextQcId = formatQcId338(fields.qcIdDigits || value);
                        ...
                        {mode === 'register' && role === 'Seeker' && (
                            <>
                                ...
                                <div className={`${formData.isQcResident ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'} p-4 rounded-xl border-2 border-dashed mt-2`}>
                                    ...
                                    <input required className="w-full p-2 bg-white border border-gray-300 rounded-lg text-sm mb-2" placeholder={formData.isQcResident ? "QC ID Number or QR payload" : "Valid ID Number or QR payload"} value={formData.qcId} onChange={handleQcIdInputChange} maxLength={20} />
