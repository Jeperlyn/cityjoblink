...
const AdminDashboard = ({
    ...
    const pendingEmployers = useMemo(
        () => employers.filter((employer) => employer.uploadedDocs && !employer.isVerified),
        [employers]
    );
    ...
    const openEmployerDocument = (employer) => {
        openDocument(
            employer?.verificationDocPath || employer?.verification_doc_path,
                        ...
                        {[
                            {
                                ...
                                value: summary.totalEmployers ?? employers.length,
                                helper: `${summary.verifiedEmployers ?? employers.filter((employer) => employer.isVerified).length} verified accounts`,
                            },
                            {
                                label: 'Employer Reviews Pending',
                                value: summary.pendingEmployerReviews ?? pendingEmployers.length,
                                helper: 'Waiting for manual document approval',
                            },
                            {
                                label: 'Seeker IDs Pending',
                                value: summary.pendingSeekerReviews ?? seekers.filter((seeker) => ['manual_review', 'not_submitted'].includes(seeker.idVerificationStatus)).length,
                                helper: 'QC IDs queued for admin checking',
                            },
                            {
                                label: 'Employer Feedback',
                                value: summary.totalFeedback ?? 0,
                                helper: 'Completed post-decision seeker ratings',
                            },
                            {
                                label: 'Average Rating',
                                value: `${Number(summary.averageRating ?? 0).toFixed(2)} / 5`,
                                helper: 'Across all submitted employer feedback',
                            },
                            {
                                label: 'Top Employer Slots',
                                value: Math.min(topEmployers.length, 10),
                                helper: 'Ranked by hires, then rating, then applications',
                            },
                        ].map((card) => (
                    ...
                    <div className="grid xl:grid-cols-[2fr,1fr] gap-6">
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex items-center justify-between gap-4">
                                <div>
                                    <h2 className="text-xl font-bold text-gray-900">Top 10 Employers</h2>
                                    <p className="text-sm text-gray-500 mt-1">Data analysis combines total hires, average feedback rating, and application volume.</p>
                                </div>
                                <span className="text-xs font-bold uppercase tracking-wide text-gray-400">Ranked</span>
                            </div>
                                ...
                                <table className="w-full text-sm text-left">
                                    ...
                                    <tbody>
                                        {topEmployers.length === 0 ? (
                                            <tr>
                                                <td colSpan="7" className="px-6 py-12 text-center text-gray-500">No employer analytics available yet.</td>
                                            </tr>
                                        ) : (
                                                ...
                                                <tr key={employer.id} className="border-b border-gray-100 hover:bg-gray-50">
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-start gap-3">
                                                            ...
                                                            <div>
                                                                <p className="font-bold text-gray-900">{employer.employerLabel}</p>
                                                                <p className="text-xs text-gray-500">{employer.email}</p>
                                                                <span className={`inline-flex mt-2 rounded-full border px-2 py-1 text-[11px] font-bold ${employer.isVerified ? 'border-green-200 bg-green-50 text-green-700' : 'border-amber-200 bg-amber-50 text-amber-700'}`}>
                                                                    {employer.isVerified ? 'Verified Employer' : 'Pending Employer'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{employer.totalJobs}</td>
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{employer.totalApplications}</td>
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{employer.hiredCount}</td>
                                                    <td className="px-6 py-4">
                                                        <div className="inline-flex items-center gap-2 rounded-full bg-amber-50 px-3 py-1 border border-amber-200 text-amber-700 font-bold">
                                                            <Star size={14} className="fill-current" />
                                                            {employer.averageRating.toFixed(2)}
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{employer.feedbackCount}</td>
                                                    <td className="px-6 py-4 font-semibold text-gray-900">{employer.conversionRate}%</td>
                                                </tr>
                                            ))
                                        )}
                                    </tbody>
                        ...
                        <div className="space-y-6">
                            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                                <h3 className="font-bold text-gray-900">Feedback Breakdown</h3>
                                ...
                                {employers.length === 0 ? (
                                    ...
                                    employers.map((employer) => (
                                        <tr key={employer.id} className="border-b border-gray-100 hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4">
                                                {employer.uploadedDocs ? (
                                                    <button onClick={() => openEmployerDocument(employer)} className="text-blue-600 flex gap-2 items-center font-bold hover:underline bg-blue-50 px-3 py-2 rounded-xl border border-blue-100">
                                                        <File size={14} />
                                                        View Docs
                                                    </button>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4">
                                                {employer.isVerified ? (
                                                    ...
                                                    <span className="bg-orange-100 text-orange-700 px-2 py-1 rounded-full text-xs font-bold border border-orange-200">Pending</span>
                                                ) : (
                                                    <span className="bg-slate-100 text-slate-700 px-2 py-1 rounded-full text-xs font-bold border border-slate-200">No Submission</span>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                                {employer.uploadedDocs ? (
                                                    <div className="flex gap-2">
                                                        ...
                                                        <button onClick={() => handleRejectClick(employer.id)} className="bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-xl text-xs font-bold transition-colors">
                                                            {employer.isVerified ? 'Revoke' : 'Reject'}
                                                        </button>
                                                    </div>
                                                ) : (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-200">
                        <div className="bg-red-50 p-6 flex flex-col items-center text-center border-b border-red-100">
                            ...
                            <p className="text-sm text-gray-600 mt-2">The employer will need to submit a fresh document set to continue.</p>
                        </div>
                        <div className="p-6 flex gap-3">
