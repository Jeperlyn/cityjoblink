...
const QC_ID_MIN_DIGITS = 14;
const QC_ID_MAX_DIGITS = 16;
...
export const parseQcQrPayload = (payload) => {
  ...
  if (qcIdDigits.length < QC_ID_MIN_DIGITS || qcIdDigits.length > QC_ID_MAX_DIGITS) {
    errors.push("QC ID from QR must contain 14 to 16 digits.");
  }
...
export const formatQcId338 = (digits) => {
  const normalized = normalizeDigits(digits).slice(0, QC_ID_MAX_DIGITS);
