...
import ResumeBuilderMain from './pages/ResumeBuilder/ResumeBuilderMain';
...
const MessagesPanel = ({ messages, user, onBack }) => (
  <div className="max-w-4xl mx-auto p-6">
    ...
    {messages.filter(m => m.toId === user.id).map(m => (
      <div key={m.id} className="p-4 border-b bg-white">
        <h4 className="font-bold">{m.senderName}</h4>
        <p>{m.content}</p>
      </div>
    ))}
  </div>
...
const App = () => {
  ...
  const renderContent = () => {
     ...
     if (currentView === 'resume-builder') {
        return <ResumeBuilderMain user={user} onBack={() => setCurrentView('seeker-dash')} />;
     }

     if (currentView === 'public-trainings') return (
        <PublicListings 
            type="trainings" 
            data={trainings} 
            user={user} 
            onRegister={(id) => {
                ...
                setTrainings(trainings.map(t => t.id===id ? {...t, slots:t.slots-1, registeredUsers:[...t.registeredUsers, user.id]} : t));
            }} 
        />
     );

     if (currentView === 'public-jobfairs') return (
        <PublicListings 
            type="jobfairs" 
            data={jobFairs} 
            user={user} 
            onRegister={(id) => {
                ...
                setJobFairs(jobFairs.map(f => f.id===id ? {...f, participants:[...f.participants, user.id]} : f));
            }} 
        />
     );
     ...
     if (currentView === 'seeker-dash') return (
        <SeekerDashboard 
            ...
            onUpdateTrainings={(id, uid, act) => setTrainings(trainings.map(t => t.id===id ? (act==='join' ? {...t, slots:t.slots-1, registeredUsers:[...t.registeredUsers, uid]} : {...t, slots:t.slots+1, registeredUsers:t.registeredUsers.filter(x=>x!==uid)}) : t))} 
            onUpdateFairs={()=>{}} 
            onReviewCompany={()=>{}} 
            onViewJob={(j) => {setSelectedJob(j); setCurrentView('job-details')}}
            // IMPORTANT: This allows the dashboard to navigate to resume builder
     ...
     if (currentView === 'employer-dash') return <EmployerDashboard 
        ...
        onUpdateProfile={handleUpdateProfile} 
        onUploadDocs={() => {
            handleUpdateProfile({...user, uploadedDocs:true}); 
            alert("Request Sent");
        }} 
        onSendMessage={()=>{}} 
